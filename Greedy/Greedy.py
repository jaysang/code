...
writer = SummaryWriter('./log/{}'.format(datetime.datetime.now().__str__().replace(':', '_'))+" Greedy")
...
for episode in range(max_episodes):
    ...
    for step in range(max_steps):
        compute_list = state[3+n_offloading:3+n_offloading+5]
        action = np.argmax(compute_list)
