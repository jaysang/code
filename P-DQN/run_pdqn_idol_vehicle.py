...
writer = SummaryWriter('./log/{}'.format(datetime.datetime.now().__str__().replace(':', '_'))+" EN")
...
def run(seed, episodes, evaluation_episodes, batch_size, gamma, inverting_gradients, initial_memory_threshold,
    ...
    # 2. 初始化环境
    env = TaskOffloadingEnv(use_en=True, use_bs=False)
    ...
    for i in tqdm(range(episodes)):
        ...
        # writer.add_scalar('episode-no-rsu/reward', episode_reward, i + 1)
        # writer.add_scalar('episode-no-rsu/avg-reward', total_reward / (i + 1), i + 1)
        # writer.add_histogram('episode-no-rsu/avg-action-param-1', np.array(action_param)[:, 0], i + 1)
        # writer.add_histogram('episode-no-rsu/avg-action-param-2', np.array(action_param)[:, 1], i + 1)
        # writer.add_histogram('episode-no-rsu/action', np.array(actions), i + 1)
