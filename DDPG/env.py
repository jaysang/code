...
class TaskOffloadingEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.resource_bs_max = np.ones(shape=self.n_bs) * 25     # GHZ
        ...
        # 最大带宽
        self.MAX_BANDWIDTH_BE = 4
        self.MAX_BANDWIDTH_BR = 2
        # 噪声功率谱密度
        self.N0 = 10
    ...
    #            np.log(1 + self.task_p[self.cur_vehicle] * self.task_h / (self.N0 * self.MAX_BANDWIDTH_BV / self.K))
    #
    #     return R_ij
    ...
    def reset(self):
        ...
        self.resource_ue_max = np.ones(shape=self.n_ue) * 0.5  # GHZ
        self.resource_en_max = np.ones(self.n_en) * 2  # GHZ
        self.resource_bs_max = np.ones(shape=self.n_bs) * 2.5  # GHZ
    ...
    def step(self, action_mix):
        ...
        """
        # print(action_mix)
        action = int(action_mix[0])
        # if random.random() < 0.2:
        #     action = random.randint(0, self.n_en + self.n_bs)
        parameters = action_mix[1]

        # action = action_mix[0]
        # NOTE：这里可以根据需要改为 action.t_comm_percent 和 action.t_comp_percent
        # print("parameters0",parameters[0])
        # print("parameters1", parameters[1])
        # print("action_space.high", self.action_space.high)
        t_comm_percent = max(min(parameters[0] / self.action_space.high+0.01, 1), 0.1)
        t_comp_percent = max(min(parameters[1] / self.action_space.high+0.01, 1), 0.1)
        print("action=",action)
        # print("t_comm_percent=",t_comm_percent)
        # print("t_comp_percent=",t_comp_percent)
    ...
    def _reward(self, action, t_comm_percent, t_comp_percent):
        if action == 0:
            ...
            if self.use_bs:
                action -= 1
                return (self._reward_rsu(action, t_comm_percent, t_comp_percent)-100)
